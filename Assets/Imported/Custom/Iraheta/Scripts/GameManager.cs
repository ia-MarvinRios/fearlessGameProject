...
public class GameManager : MonoBehaviour
{
    ...
    public void RecolectarItem(string tagMision, int cantidad)
    {
        for (int i = 0; i < misiones.Count; i++)
        {
            if (misiones[i].selectedTag == tagMision && !misiones[i].EstaCompleta)
            {
                ...
                if (uIController.gridContainer) uIController.AgregarMisionesAlGrid();
    ...
    {/*
        ...
        {
            Debug.Log("Asignar mas velocidad: Tecla -> V");
            Debug.Log("Asignar mas fuerza: Tecla -> F");
        }
        */
...
public class GameManagerEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        for (int i = 0; i < misionesProp.arraySize; i++)
        {
            ...
            SerializedProperty iconoProp = misionProp.FindPropertyRelative("_icono");
            ...
            // Campo para el icono
            iconoProp.objectReferenceValue = EditorGUILayout.ObjectField("Ícono", iconoProp.objectReferenceValue, typeof(Sprite), false);
