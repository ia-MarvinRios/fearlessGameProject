...
public class IrahetaTools
{
    ...
    [MenuItem("Iraheta Tools/PLAYER/PlayerAttackController")]
    private static void AddAttackPlayerController()
    {
        if (Selection.activeGameObject != null && Selection.activeGameObject.CompareTag("Player"))
        {
            ...
            if (selectedObject.GetComponent<PlayerAttackController>() == null)
            {
                selectedObject.AddComponent<PlayerAttackController>();
                Debug.Log("PlayerAttackController añadido a: " + selectedObject.name);
            }
            else
            {
            ...
            if (selectedObject.GetComponent<PlayerActionsController>() == null)
            {
                selectedObject.AddComponent<PlayerActionsController>();
            }
    ...
    [MenuItem("Iraheta Tools/Mapeo/Asignar Tecla 'i' para información ")]
    private static void CrearMapa()
    {
        Debug.Log("No implementado yet");
    }
    ...
    */
        ...
        if (Selection.activeGameObject != null && !Selection.activeGameObject.CompareTag("Player"))
        {
            ...
            if (selectedObject.GetComponent<NpcController>() == null)
            {
                ...
                if (sphereCollider == null)
                {
                    // Añadir SphereCollider si no existe
                    sphereCollider = selectedObject.AddComponent<SphereCollider>();
                    sphereCollider.radius = 3f; // Radio de 2 metros
                    sphereCollider.isTrigger = true;
    ...
    [MenuItem("Iraheta Tools/MISIONES/Completada")]
    private static void onMisiones()
    {
        ...
        if (Selection.activeGameObject != null && !Selection.activeGameObject.CompareTag("Player"))
        {
            ...
            if (selectedObject.GetComponent<MissionComplete>() == null)
            {
                
                selectedObject.AddComponent<MissionComplete>();
                ...
                GameManager _instancia = GameObject.Find("GameManager").GetComponent<GameManager>();
                selectedObject.GetComponent<MissionComplete>().gameManager = _instancia;

            }
            else
            {
                Debug.LogWarning("El objeto ya es Mision Completada");
            }
        ...
        if (Selection.activeGameObject != null && !Selection.activeGameObject.CompareTag("Player"))
        {
            ...
            if (selectedObject.GetComponent<ExpController>() == null)
            {
                ...
                if (sphereCollider == null)
                {
                    // Añadir SphereCollider si no existe
                    sphereCollider = selectedObject.AddComponent<SphereCollider>();
                    sphereCollider.radius = 1.5f; // Radio de 0.5 metros
                    sphereCollider.isTrigger = true;
    ...
    public static void AddLayer(string layerName)
    {
        ...
        {
            Debug.LogError("El nombre de la capa no puede estar vacío.");
            return;
        }
        ...
        for (int i = 8; i < layersProp.arraySize; i++) // Las capas 0-7 son reservadas
        {
            SerializedProperty layer = layersProp.GetArrayElementAtIndex(i);
            if (layer.stringValue == layerName)
            {
                Debug.LogWarning($"La capa '{layerName}' ya existe.");
                return;
            }
            ...
            if (string.IsNullOrEmpty(layer.stringValue))
            {
                ...
                tagManager.ApplyModifiedProperties();
                Debug.Log($"Capa '{layerName}' agregada exitosamente.");
                return;
            }
        }

        Debug.LogError("No hay espacios disponibles para agregar una nueva capa.");
    }
...
public class ConvertirEnemigoWindow : EditorWindow
{
    ...
    [MenuItem("Iraheta Tools/EVENTOS/Agregar Interacciones")]
    private static void AgregarInteraccion()
    {
        if (Selection.activeGameObject != null)
        {
            GameObject selectedObject = Selection.activeGameObject;

            if (selectedObject.GetComponent<Interaccion>() == null)
            {
                selectedObject.AddComponent<Interaccion>();
                Debug.Log("Se ha agregado el script de Interaccion");
    ...
    [MenuItem("Iraheta Tools/EVENTOS/Agregar Metodo Signal")]
    private static void signalEvents()
    {
        if (Selection.activeGameObject != null)
        {
            GameObject selectedObject = Selection.activeGameObject;

            if (selectedObject.GetComponent<SignalsEvents>() == null)
            {
                selectedObject.AddComponent<SignalsEvents>();
                Debug.Log("Se ha agregado el script de SignalsEvents");
    ...
    private static void CrearEnemigo()
    {
        if (Selection.activeGameObject != null && !Selection.activeGameObject.CompareTag("Player"))
        {
            ...
            {
                selectedObject.tag = "ENEMIGO";
                selectedObject.layer = LayerMask.NameToLayer("ENEMIGO");
                selectedObject.AddComponent<EnemyController>();

                Debug.Log("Se ha agregado el script de EnemyController");
            ...
            if (sphereCollider == null)
            {
                ...
                // Añadir el componente Animator si no existe
                Animator animator = selectedObject.GetComponent<Animator>();
                if (animator != null)
                {
                    // Buscar el Animator Controller en la carpeta Assets/Resources/Animators
                    string originalPath = "Assets/Custom/Iraheta/Resources/Controllers/EnemyAnimatorController.controller";


                    // Crear una copia en la misma carpeta con un nombre único
                    string copiedPath = $"Assets/Custom/Iraheta/Resources/Controllers/{selectedObject.name}_Animator.controller";
                    ...
                    if (!File.Exists(copiedPath))
                    {
                        ...
                        // Importar la copia al proyecto para que Unity la reconozca
                        AssetDatabase.ImportAsset(copiedPath);
                        AssetDatabase.Refresh();

                    // Directorio de destino y nuevo nombre del archivo
                    string targetDirectory = "Assets/Custom/Iraheta/Resources/Controllers/";
                    string newFileName = selectedObject.name + "_Animator.controller";
                    string newFilePath = targetDirectory + newFileName;


                    AnimatorController copiedController = AssetDatabase.LoadAssetAtPath<AnimatorController>(newFilePath);
                    if (copiedController != null)
                    {
                        copiedController.name = newFileName.Replace(".controller", ""); // Ajustar el nombre interno
                        EditorUtility.SetDirty(copiedController); // Marcarlo como modificado
                        AssetDatabase.SaveAssets();
                    }

                }

                // Cargar el Animator Controller desde la carpeta Resources
                RuntimeAnimatorController animatorController = Resources.Load<RuntimeAnimatorController>($"Controllers/{selectedObject.name}_Animator");



                // Asignar el Animator Controller al Animator
                animator.runtimeAnimatorController = animatorController;

                    // Mensaje de éxito
                    Debug.Log("Animator Controller asignado correctamente.");
